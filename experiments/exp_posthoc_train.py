#!/usr/bin/env python3
"""
Phase 13 — Experiment 1: Training with Per-Step Gradient Logging
=================================================================

Trains two arms with per-step logging of per-sample gradient norms and
incoherent norms. Logs are saved to parquet for use in Experiment 2
(post-hoc certificate computation).

Arms:
  dist_aware_log   — DP-SGD with reduced noise (sigma calibrated to C_eff=0.4C)
  pda_dpmd_da_log  — Same reduced noise + PDA-DPMD public gradient blending

For each step t and each sampled example j:
  Logs: step, example_idx, grad_norm, incoherent_norm, beta

Subspace V: top-100 PCA directions of public gradient covariance (fixed).

Usage
-----
  python experiments/exp_posthoc_train.py --arm dist_aware_log --gpu 0
  python experiments/exp_posthoc_train.py --arm pda_dpmd_da_log --gpu 0
  python experiments/exp_posthoc_train.py --gpu 0   # both arms
"""
...
# ---------------------------------------------------------------------------
# Constants (match P11 exactly)
# ---------------------------------------------------------------------------

DELTA           = 1e-5
EPS             = 2.0
CLIP_C          = 1.0
C_EFF_RATIO     = 0.4          # C_eff = C_EFF_RATIO * CLIP_C
RANK_V          = 100
N_PUB           = 2000
EPOCHS          = 60
BATCH_SIZE      = 1000
...
PUB_BATCH       = 256
GRAD_CHUNK      = 64
I_FACTOR        = 8            # Amid cosine alpha schedule
DATA_ROOT       = "./data"
...
# ---------------------------------------------------------------------------
# Data helpers (same split as P11, seed=42)
# ---------------------------------------------------------------------------
...
def _build_datasets(data_root, seed=42):
    ...
    # Private dataset with augmentation + index tracking
...
def _pretrain_on_public(model, pub_x, pub_y, device):
    ...
    for ep in range(1, PRETRAIN_EPOCHS + 1):
        ...
        for i in range(0, N, PUB_BATCH):
            ...
            opt.step()
        sch.step()
    print(f"[P13-E1] Pretraining done ({PRETRAIN_EPOCHS} ep)")
...
def _compute_subspace(model, pub_x, pub_y, device, rank=RANK_V):
    print(f"[P13-E1] Computing public PCA subspace (rank={rank})...")
    model.eval()
    parts = []
    ...
    V = V[:, :k].cpu()
    print(f"[P13-E1] Subspace V: {V.shape}")
    del G; torch.cuda.empty_cache()
    return V  # [d, rank]


# ---------------------------------------------------------------------------
# Public gradient (for pda_dpmd_da_log)
# ---------------------------------------------------------------------------
...
def _pub_grad_flat(model, pub_x, pub_y, device):
    ...
    for i in range(0, N, PUB_BATCH):
        ...
        total_loss = total_loss + F.cross_entropy(model(xb), yb, reduction="sum")
...
def _save_log(log_steps, log_idx, log_gnorm, log_inorm, out_path):
    """Save log arrays to parquet (fallback: npz)."""
    try:
        import pandas as pd
        df = pd.DataFrame({
        ...
        print(f"[P13-E1] Log saved: {out_path} ({len(df):,} rows, "
        ...
        print(f"[P13-E1] Log saved (npz fallback): {npz_path}")
...
def _train_run(arm_name, eps, seed, priv_ds, test_ds,
               pub_x, pub_y, device, out_dir, log_dir):
    ...
    c_eff     = C_EFF_RATIO * CLIP_C          # 0.4
    sigma_use = sigma_van * c_eff             # absolute noise std

    print(f"[P13-E1] T={T_steps}, q={q:.5f}, sigma_van={sigma_van:.4f}, "
          f"c_eff={c_eff:.3f}, sigma_use={sigma_use:.4f}")
    print(f"[P13-E1] Noise std: {sigma_use:.4f}  "
          f"(vs vanilla {sigma_van*CLIP_C:.4f})")

    # Model
    model = _make_model().to(device)
    d     = _num_params(model)
    print(f"[P13-E1] Model: {d:,} params")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, y, ex_idx in priv_loader:
            ...
            # Per-sample gradients + clip + log (chunked)
            ...
            for ci in range(0, B, GRAD_CHUNK):
                ...
                norms  = gc.norm(dim=1, keepdim=True).clamp(min=1e-8)    # [c, 1]
                gc_clip = gc * (CLIP_C / norms).clamp(max=1.0)            # [c, d]
                sum_g  += gc_clip.sum(0)

                # Incoherent projection and logging (no extra memory: scalars only)
                with torch.no_grad():
            ...
            # DP update (same structure as P11)
            ...
            if "pda" in arm_name:
                ...
                # Normalize g_pub to private scale
        ...
        row = {"epoch": epoch, "train_loss": f"{total_loss/n_batches:.4f}",
        ...
        # Flush log to disk every 10 epochs to avoid OOM
        if epoch % 10 == 0:
    ...
    print(f"[P13-E1] Done — final={test_acc:.4f}  best={best_acc:.4f}  "
...
def main():
    ...
    parser.add_argument("--out_dir",   type=str,   default=EXP1_DIR)
    ...
    print(f"[P13-E1] Device: {device}")

    pub_ds, priv_ds, test_ds, pub_x, pub_y = _build_datasets(args.data_root)
    print(f"[P13-E1] Dataset: {len(priv_ds)} private, {len(pub_ds)} public")

    arms = [args.arm] if args.arm else ARMS
    for arm in arms:
        _train_run(arm, args.eps, args.seed, priv_ds, test_ds,
                   pub_x, pub_y, device, args.out_dir, args.log_dir)
